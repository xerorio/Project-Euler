...
# bottom row of grid is int(grid[0]
# second row from the bottom is int(grid[1] and so on
...
def calc_diagonal() -> int:
    ...
    for i in range(17):
        for j in range(17):
            product = int(grid[j][i]) * int(grid[j + 1][i + 1]) * int(grid[j + 2][i + 2]) * int(grid[j + 3][i + 3])
            if (product > largest_product):
                largest_product = product
    # other diagonals
    for i in range(3, 20):
